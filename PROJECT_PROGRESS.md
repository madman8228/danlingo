...
- Current status:
  - Exercise Review now uses a single primary action: 批准并生效.
  - Already-active questions no longer show a second activation button; they display 当前题 instead.
- Blocked / not yet done:
...
- 官方单文件模板现在只主推知识资产字段，不再示例手写 quiz。
...
- `Exercise Review` 现在主推 `生成翻译选择题 / 生成判断题`，不再把 `word_card` 当成主流程题型。
- 学习端已正式支持 `translation_choice`；legacy `word_card / multiple_choice` 会在读取时兼容迁移。

## Not Done
- 学习端还没有新增独立词卡组件；`translation_choice` 仍复用现有选择题组件。
...
- Current status:
  ...
  - `Exercise Review` now treats `translation_choice` as the formal word-meaning exercise type and uses explicit buttons: `生成翻译选择题 / 生成判断题`.
...
- Current status:
  - Browser-level verification on `/lesson-exercise/vocab-course-...` confirmed the learner does not actually mark every option correct; wrong selections still produce `再试一次`.
